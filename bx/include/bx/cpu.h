...
#	endif
#else
		int64_t tmp = *_ptr; *_ptr += _add; return tmp;
		//return __sync_fetch_and_add(_ptr, _add);
#endif // BX_COMPILER_
	}
	...
	inline int64_t atomicFetchAndSub<int64_t>(volatile int64_t* _ptr, int64_t _sub)
	{
		...
		return atomicFetchAndAdd(_ptr, -_sub);
#else
		int64_t tmp = *_ptr; *_ptr -= _sub; return tmp;
		//return __sync_fetch_and_sub(_ptr, _sub);
#endif // BX_COMPILER_
	}
...
#else
		volatile int64_t* ptr = (volatile int64_t*)_ptr;
		if ( *ptr == _old )
		{
			*ptr = _new;
			return true;
