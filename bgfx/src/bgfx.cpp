...
#undef CAPS_FLAGS
	...
	bool Context::init(RendererType::Enum _type)
	{
		...
		printf( "Context::init - 2\n" );
		m_declRef.init();
		printf( "Context::init - 3\n" );

		CommandBuffer& cmdbuf = getCommandBuffer(CommandBuffer::RendererInit);
		cmdbuf.write(_type);

		printf( "Context::init - 4\n" );
		frameNoRenderWait();

		printf( "Context::init - 5\n" );
...
#undef CHECK_HANDLE_LEAK
	...
	void Context::swap()
	{
		...
		printf( "Context::swap 2\n" );
		m_submit->finish();
		printf( "Context::swap 3\n" );
		bx::xchg(m_render, m_submit);
		printf( "Context::swap 4\n" );
		memcpy(m_render->m_occlusion, m_submit->m_occlusion, sizeof(m_submit->m_occlusion) );

		if (!BX_ENABLED(BGFX_CONFIG_MULTITHREADED)
		||  m_singleThreaded)
		{
			printf( "Context::swap 5\n" );
			renderFrame();
		}

		printf( "Context::swap 6\n" );

		m_frames++;
		m_submit->start();

		printf( "Context::swap 7\n" );

		memset(m_seq, 0, sizeof(m_seq) );
		freeAllHandles(m_submit);

		printf( "Context::swap 8\n" );
	...
	{
		printf( "Context::renderFrame() 1\n" );
		BGFX_PROFILER_SCOPE(bgfx, render_frame, 0xff2040ff);

		if (m_rendererInitialized
		&& !m_flipAfterRender)
		{
			printf( "Context::renderFrame() 2\n" );
			m_renderCtx->flip(m_render->m_hmd);
		}

		printf( "Context::renderFrame() 3\n" );
		gameSemWait();

		printf( "Context::renderFrame() 4\n" );
		rendererExecCommands(m_render->m_cmdPre);
		if (m_rendererInitialized)
		{
			printf( "Context::renderFrame() 5\n" );
			BGFX_PROFILER_SCOPE(bgfx, render_submit, 0xff2040ff);
			m_renderCtx->submit(m_render, m_clearQuad, m_textVideoMemBlitter);
		}
		printf( "Context::renderFrame() 6\n" );
		rendererExecCommands(m_render->m_cmdPost);

		printf( "Context::renderFrame() 7\n" );
		renderSemPost();

		if (m_rendererInitialized
		&&  m_flipAfterRender)
		{
			printf( "Context::renderFrame() 8\n" );
			m_renderCtx->flip(m_render->m_hmd);
		}

		printf( "Context::renderFrame() 9\n" );
		return m_exit;
	}
...
#undef BGFX_RENDERER_CONTEXT
	...
	bool init(RendererType::Enum _type, uint16_t _vendorId, uint16_t _deviceId, CallbackI* _callback, bx::AllocatorI* _allocator)
	{
		...
		bgfx::s3eSetWindow( s3eGLGetNativeWindow() );
#endif
		printf( "init begin 2\n" );
